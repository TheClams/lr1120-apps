...
use lr1120::{
    fsk::{AddrComp, Crc, DcFree, FskPktFormat, PblLenDetect, PulseShape, RxBw},
    radio::{PacketType, RampTime},
    status::{IRQ_MASK_RX_DONE, IRQ_MASK_TX_DONE, Intr},
    system::{ChipMode, DioNum, DioRfSwitchCfg}
};

const PLD_SIZE : u8 = 16;
...
async fn main(spawner: Spawner) {
    ...
    // 901MHz, 0dbM, 250kb/s, h=0.5, 32b syncword with 2B CRC
    ...
    lr1120.set_packet_type(PacketType::Gfsk).await.expect("Setting packet type");
    lr1120.set_fsk_modulation(100_000, PulseShape::Bt0p5, RxBw::Bw467000, 25_000).await.expect("Setting packet type");
    lr1120.set_fsk_syncword(0xCD05DEAD).await.expect("SetSyncword");
    lr1120.set_fsk_packet(8, PblLenDetect::None, 32,AddrComp::Off, FskPktFormat::Variable8bit, PLD_SIZE, Crc::Crc2Byte, DcFree::DcFreeWhitening).await.expect("SetSyncword");
    lr1120.set_tx_params(0, RampTime::Ramp16u).await.expect("Setting TX parameters");

    lr1120.set_dio_irq(Intr::new(IRQ_MASK_TX_DONE|IRQ_MASK_RX_DONE), Intr::new(0)).await.expect("Setting DIO7 as IRQ");
    ...
    if intr.rx_done() {
        ...
        if intr.rx_error() {
            ...
            info!("[RX] Payload = {:02x} | intr={:08x} | RSSI=-{}dBm, SNR={}.{:02}",
                // pld,
                lr1120.buffer()[..(nb_byte+3) as usize],
                intr.value(),
